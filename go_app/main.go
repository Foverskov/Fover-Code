...
func main() {
	...
	tools := []ToolDefinition{ReadFileDefinition, ListFilesDefinition, EditFileDefinition, RunBashDefinition}
...
var RunBashDefinition = ToolDefinition{
	Name:        "run_bash",
	Description: "Run a bash command/script",
...
func runBash(input json.RawMessage) (string, error) {
	runBashInput := RunBashInput{}
	err := json.Unmarshal(input, &runBashInput)
